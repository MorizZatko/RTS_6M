"""Blender multishot automation.

This module first deletes ervery object with "ML_" in its name, spawns a basic cube with "ML_" name at location (0, 0, 0).
It creates a camera and a custom light setup, links all to the active collection,
and configures their individual parameters.
Finally, it renders three shots to a hardcoded directory, each with individual values for camera position and key light energy.
- Camera: 85mm lens, clip start/end set to 0.1 - 100.0.
- Key-Light: Area light with RGB color (1, 1, 1) and different power values.
...
# Setup parameters for each render shot
shots = [{"name": "close_up", "cam_pos": (5, -5, 5), "light_energy": 3000},
        {"name": "wide_shot", "cam_pos": (0, -20, 5), "light_energy": 5000},
        {"name": "top_angle", "cam_pos": (-20, -20, 20), "light_energy": 1000}]
...
# Track rim light to cube object
...
# Select key light
key_light = bpy.data.objects.get("ML_Key")
bpy.context.view_layer.objects.active = key_light

# Render each shot with custom name
for shot in shots:
    cam_obj.location = shot["cam_pos"]
    key_light.data.energy = shot["light_energy"]
    filename = f"{shot['name']}.png"
    bpy.context.scene.camera = cam_obj
    bpy.context.scene.render.filepath = os.path.join(r"C:\Users\moriz\Desktop\RTS\python_start\Media_Doc\BPY_Mulitshot", filename)
